


// import React from "react";
// import "./Navbar.css";
// import Signin from "../Pages/Signin/Signin";
// import { Link } from "react-router-dom";

// const MobNavbar = () => {
//   return (
//     <>
//       <div className="navbar-container">
//         <div className="container-fluid nav-bar px-0">
//           <div className="container pt-1">
//             <nav className="navbar navbar-expand-lg">
//               <Link to="/" className="navbar-brand p-0 media-nav-logo-desk">
//                 <img
//                   src="https://i.ibb.co/rfNr9jq/oshopinuloo.png"
//                   width={150}
//                   alt="Logo"
//                 />
//               </Link>
//               <button
//                 className="navbar-toggler text-light"
//                 data-bs-toggle="offcanvas"
//                 href="#offcanvasNavbar"
//                 role="button"
//                 aria-controls="offcanvasNavbar"
//               >
//                 <span className="fa fa-bars" />
//               </button>
//               <div
//                 className="offcanvas offcanvas-start navbar-offcanvas"
//                 tabIndex="-1"
//                 id="offcanvasNavbar"
//                 aria-labelledby="offcanvasNavbarLabel"
//               >
//                 <div className="offcanvas-header">
//                   <button
//                     type="button"
//                     className="btn-close text-reset"
//                     data-bs-dismiss="offcanvas"
//                     aria-label="Close"
//                   ></button>
//                 </div>
//                 <div className="offcanvas-body">
//                   <a href="#" className="sign-in-button">
//                     <i className="fas fa-sign-in-alt me-2"></i> Sign In
//                   </a>
//                   <p className="contact-number">Contact: 123-456-789</p>
//                   <ul className="navbar-nav">
//                     <li className="nav-item">
//                       <Link to="/tile-and-stone" className="nav-link">
//                        Insurence Products <span className="fa fa-chevron-right"></span>
//                       </Link>
//                     </li>
//                     <li className="nav-item">
//                       <Link to="/new-and-inspiring" className="nav-link">
//                        Renew Your Policy <span className="fa fa-chevron-right"></span>
//                       </Link>
//                     </li>
//                     <li className="nav-item">
//                       <Link to="/vinyl" className="nav-link">
//                         Resources <span className="fa fa-chevron-right"></span>
//                       </Link>
//                     </li>
                   
//                   </ul>
//                 </div>
//               </div>
//             </nav>
//           </div>
//         </div>
        
//         {/* Sign In Modal */}
//         <div className="modal fade" id="signinModal" tabIndex="-1" aria-hidden="true">
//           <div className="modal-dialog modal-dialog-centered" style={{ maxWidth: "400px" }}>
//             <div className="modal-content">
//               <Signin />
//             </div>
//           </div>
//         </div>
//       </div>
//     </>
//   );
// };

// export default MobNavbar;






import React, { useState } from "react";
import "./Navbar.css";
// import Signin from "../Pages/Signin/Signin";
import { Link } from "react-router-dom";

const MobNavbar = () => {
  const [activeDropdown, setActiveDropdown] = useState(null);

  // Toggle dropdown for a specific menu item
  const toggleDropdown = (menu) => {
    setActiveDropdown(activeDropdown === menu ? null : menu);
  };

  return (
    <>
      <div className="navbar-container">
        <div className="container-fluid nav-bar px-0">
          <div className="container pt-1">
            <nav className="navbar navbar-expand-lg">
              <Link to="/" className="navbar-brand p-0 media-nav-logo-desk">
                <img
                  src="https://i.ibb.co/rfNr9jq/oshopinuloo.png"
                  width={150}
                  alt="Logo"
                />
              </Link>
              <button
  className="navbar-toggler text-light"
  data-bs-toggle="offcanvas"
  href="#offcanvasNavbar"
  role="button"
  aria-controls="offcanvasNavbar"
>
  <span className="fa fa-bars" />
</button>
<div
  className="offcanvas offcanvas-end navbar-offcanvas"
  tabIndex="-1"
  id="offcanvasNavbar"
  aria-labelledby="offcanvasNavbarLabel"
>
  <div className="offcanvas-header">
    <img
      src="https://i.ibb.co/rfNr9jq/oshopinuloo.png"
      alt="Logo"
    />
    <button
      type="button"
      className="text-reset"
      data-bs-dismiss="offcanvas"
      aria-label="Close"
    >
      <i className="fa-solid fa-xmark"></i>
    </button>
  </div>
  <div className="offcanvas-body">
    {/* <p className="contact-number">Contact: 123-456-789</p> */}
    <ul className="navbar-nav">
      <li className="nav-item">
        <a
          href="#"
          className="nav-link"
          onClick={() => toggleDropdown("insurance")}
        >
          Insurance Products{" "}
          <span
            className={`fa-solid ${
              activeDropdown === "insurance" ? "fa-minus" : "fa-plus"
            }`}
          ></span>
        </a>
        {activeDropdown === "insurance" && (
          <ul className="submenu">
            <li className="mobile-navbar-link-to-icon">
              <Link to="/motor-insurance" className="submenu-item">
                Motor Insurance
              </Link>
              <i class="fa-solid fa-angle-right"></i>
            </li>
            <li className="mobile-navbar-link-to-icon">
              <Link to="/life-insurance" className="submenu-item">
                Life Insurance
              </Link>
              <i class="fa-solid fa-angle-right"></i>
            </li>
            <li className="mobile-navbar-link-to-icon">
              <Link to="/medical-insurance" className="submenu-item">
                Medical Insurance
              </Link>
              <i class="fa-solid fa-angle-right"></i>
            </li>
            <li className="mobile-navbar-link-to-icon">
              <Link to="/travel-insurance" className="submenu-item">
                Travel Insurance
              </Link>
              <i class="fa-solid fa-angle-right"></i>
            </li>
          </ul>
        )}
      </li>
      <li className="nav-item">
        <a
          href="#"
          className="nav-link"
          onClick={() => toggleDropdown("renewPolicy")}
        >
          Renew Your Policy{" "}
          <span
            className={`fa-solid ${
              activeDropdown === "renewPolicy" ? "fa-minus" : "fa-plus"
            }`}
          ></span>
        </a>
        {activeDropdown === "renewPolicy" && (
          <ul className="submenu">
            <li className="mobile-navbar-link-to-icon">
              <a  href="#" className="submenu-item">
                Auto Renewal
              </a>
              <i class="fa-solid fa-angle-right"></i>
            </li>
            <li className="mobile-navbar-link-to-icon">
              <a href="#" className="submenu-item">
                Life Policy Renewal
              </a>
              <i class="fa-solid fa-angle-right"></i>
            </li>
            <li className="mobile-navbar-link-to-icon">
              <a href="#" className="submenu-item">
                Health Policy Renewal
              </a>
              <i class="fa-solid fa-angle-right"></i>
            </li>
          </ul>
        )}
      </li>
      <li className="nav-item">
        <a
          href="#"
          className="nav-link"
          onClick={() => toggleDropdown("resources")}
        >
          Resources{" "}
          <span
            className={`fa-solid ${
              activeDropdown === "resources" ? "fa-minus" : "fa-plus"
            }`}
          ></span>
        </a>
        {activeDropdown === "resources" && (
          <ul className="submenu">
            <li className="mobile-navbar-link-to-icon">
              <a href="#" className="submenu-item">
                Blog
              </a>
              <i class="fa-solid fa-angle-right"></i>
            </li>
            <li className="mobile-navbar-link-to-icon">
              <a href="#" className="submenu-item">
                FAQ
              </a>
              <i class="fa-solid fa-angle-right"></i>
            </li>
            <li className="mobile-navbar-link-to-icon">
              <a href="#" className="submenu-item">
                Support
              </a>
              <i class="fa-solid fa-angle-right"></i>
            </li>
          </ul>
        )}
      </li>
    </ul>
    <div className="d-flex n-t-m-m-f">
      <a href="#exampleModalTogglemedia" data-bs-toggle="modal"  className="sign-in-button login-2">
        <i  className="me-2"></i> Signup
      </a>
      <a href="#" className="sign-in-button talk-t-u">
        <i className="fas fa-headset me-2 dark-to-logo"></i> Talk to us
      </a>
    </div>
  </div>  
</div>

            </nav>
          </div>
        </div>

        <div class="modal fade" id="exampleModalTogglemedia" aria-hidden="true" aria-labelledby="exampleModalToggleLabel" tabindex="-1">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      {/* <div class="modal-header">
        <h5 class="modal-title" id="exampleModalToggleLabel">Modal 1</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div> */}
      <div class="modal-body">
      <div>
            <div className="card rounded-4 border-0">
                <div className="card-header  border-0 d-flex" style={{}}>
                    <h5 className="card-title mb-0">Sign-in</h5>
                    <span className="close-icon ms-auto">
                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                    </span>
                </div>
                <div className="card-body">
                    <div className="input-group mb-3">
                        <small className="text-muted mb-3 fw-bold text-secondary">
                            For a more personalised experience, sign in with your registered mobile
                            number
                        </small>
                        <span className="input-group-text">
                            <img
                                src="data:image/png;base64,"
                                style={{ width: 30 }}
                                alt="Dubai flag"
                                className="me-2"
                            />{" "}
                            +91
                        </span>
                        <input type="text" className="form-control" placeholder="Mobile Number" />
                    </div>
                    <button data-bs-target="#exampleModalToggle2media" style={{border:'1px solid #003d2b'}} data-bs-toggle="modal" data-bs-dismiss="modal" className="btn btn-primary w-100 mb-3">Sign in with OTP</button>
                    <div className="or-divider text-center">
                        <span className='p-2 mb-3'>or</span>
                    </div>
                    <button className="btn btn-success w-100 mb-3">
                        <i className="fab fa-whatsapp me-2" /> Sign in with WhatsApp
                    </button>
                    <button className="btn btn-light w-100">
                        <img
                            src="data:image/png;base64,"
                            style={{ width: 30 }}
                            alt="Google logo"
                            className="me-2"
                        />
                        Sign in as kuldeepkumar
                    </button>
                </div>
            </div>
        </div>
      </div>
      <div class="modal-footer">
        {/* <button class="btn btn-primary" data-bs-target="#exampleModalToggle2" data-bs-toggle="modal" data-bs-dismiss="modal">Open second modal</button> */}
      </div>
    </div>
  </div>
</div>
<div class="modal fade" id="exampleModalToggle2media" aria-hidden="true" aria-labelledby="exampleModalToggleLabel2" tabindex="-1">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-body">
        {/* Hide this modal and show the first with the button below. */}
        <div className="modal-dialog modal-dialog-centered">
                <div className="modal-content">
                    {/* Modal Header */}
                    <div className="modal-header">
                        <h5 className="modal-title" id="otpModalLabel">
                            Verify OTP
                        </h5>
                        <button
                            type="button"
                            className="btn-close"
                            data-bs-dismiss="modal"
                            aria-label="Close"
                        ></button>
                    </div>

                    {/* Modal Body */}
                    <div className="modal-body">
                        <p>Please enter the OTP you’ve received on <b>9578292443</b></p>
                        <p>
                            <a href="#" className="text-primary mycolor">Change mobile number</a>
                        </p>
                        <div className="mb-3">
                            <input
                                type="text"
                                className="form-control"
                                placeholder="Enter OTP"
                                maxLength="6"
                            />
                        </div>
                        <p className="text-muted">
                            Didn’t receive the OTP yet? 
                            <a href="#" className="text-primary mycolor"> Resend in 24s</a>
                        </p>
                    </div>

                    {/* Modal Footer */}
                    <div className="modal-footer">
                        <button style={{border:'1px solid #003d2b'}} type="button" className="btn verify-btn-clr btn-primary w-100">
                            Verify mobile number
                        </button>
                        <small className="text-muted d-block text-center mt-2">
                            ©OTP has been sent on your entered number.
                        </small>
                    </div>
                </div>
            </div>
      </div>
      <div class="modal-footer">
        <button style={{border:'1px solid #003d2b'}} class="btn btn-primary verify-btn-clr" data-bs-target="#exampleModalTogglemedia" data-bs-toggle="modal" data-bs-dismiss="modal">Back</button>
      </div>
    </div>
  </div>
</div>
      </div>
    </>
  );
};

export default MobNavbar;
